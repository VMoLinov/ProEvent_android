package ru.myproevent.domain.utils.temp;
...
public class Cache {
    ...
    public File saveImgToCache(Bitmap bitmap, @Nullable String name) {
        ...
        try {
            cachePath = new File(ProEventApp.Companion.getInstance().getApplicationContext().getCacheDir(), CHILD_DIR);
            cachePath.mkdirs();
    ...
    @Nullable public Uri getUriByFileName(String name) {
        Context context = ProEventApp.Companion.getInstance().getApplicationContext();
        String fileName;
        if (!TextUtils.isEmpty(name)) {
            fileName = name;
        } else {
    ...
    public String getContentType(Uri uri) {
        return ProEventApp.Companion.getInstance().getApplicationContext().getContentResolver().getType(uri);
    }
